...
const Cta = () => {
  ...
  // Background video component with mobile/desktop conditional rendering
  const BackgroundVideo = ({ videoId, title }: { videoId: string, title: string }) => (
    <div className="absolute inset-0 w-full h-full overflow-hidden">
      {!isMobile ? (
        <iframe 
          src={`https://player.vimeo.com/video/${videoId}?h=ff2bc9aa48&badge=0&autopause=0&player_id=0&app_id=58479&background=1&autoplay=1&loop=1&muted=1`} 
          frameBorder="0" 
  ...
  // Styled section with video background
  const StyledSection = ({ 
    videoId, 
    overlayVideoId, 
    title, 
    ...
    <section className="py-24 relative overflow-hidden bg-black/70">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <BackgroundVideo videoId={videoId} title={`Background Video Base - ${title}`} />
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden border-2 border-gold-500/80 shadow-[0_0_10px_3px_rgba(255,195,0,0.5)] rounded-md z-15">
          {!isMobile && (
            <iframe 
              src={`https://player.vimeo.com/video/${overlayVideoId}?h=96cbb5c847&badge=0&autopause=0&player_id=0&app_id=58479&background=1&autoplay=1&loop=1&muted=1`} 
              frameBorder="0" 
              allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" 
              className="absolute w-[150%] h-[150%] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 object-cover min-w-[150%] min-h-[150%]" 
  ...
  return (
    <>
      <StyledSection 
        ...
        description="Join thousands of forward-thinking people who believe in AllWillRetire."
      >
      ...
      <StyledSection 
        ...
        title="Future" 
        heading="Secure your" 
        description="Financial freedom is not about having money, its about having choices."
      >
        {/* Empty child div for spacing */}
      ...
      <StyledSection 
        ...
        title="Purpose" 
        heading="In life we need" 
        description="Join our movement and start believing."
      >
