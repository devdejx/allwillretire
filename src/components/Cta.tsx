...
const Cta = () => {
  ...
  // Helper function to render video or image based on device
  const renderBackgroundMedia = (videoId, fallbackImageSrc) => {
    if (isMobile) {
      return (
        <OptimizedImage 
  ...
  const renderSection = (videoId, overlayVideoId, title, subtitle, buttonContent = null, imageUrl = "/lovable-uploads/79de4c21-363e-44ae-9950-2529769150cc.png", overlayImageUrl = "/lovable-uploads/82199989-9426-49c0-b494-3fe93ddaac2a.png") => (
    <section className="py-24 relative overflow-hidden bg-black/70">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="absolute inset-0 w-full h-full overflow-hidden">
          {renderBackgroundMedia(videoId, imageUrl)}
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden border-2 border-gold-500/80 shadow-[0_0_10px_3px_rgba(255,195,0,0.5)] rounded-md z-15">
          {renderBackgroundMedia(overlayVideoId, overlayImageUrl)}
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 relative z-40 text-white pt-0">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-300 mb-8 max-w-xl mx-auto text-xs">
            {subtitle === "Prosperity" ? "Join thousands of forward-thinking people who believe in AllWillRetire." : 
             subtitle === "Future" ? "Financial freedom is not about having money, its about having choices." : 
